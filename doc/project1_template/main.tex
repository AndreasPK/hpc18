...
According to the roofline model this would require bandwith close to $$13 GFLOPS * 0,31Flop/Byte = 3,4GB/s$$
However actual bandwith used for the benchmark is at 3,45B/S.
...
Immediatly obvious from the snippet above we get $nclusters * nfeatures$ iterations.
...
It is however important to point out this does assume NO caching. In practice much of the memory pressure will be absorbed by the cache.
...
When considering the maximum realistic performance of the given code there are multiple things to consider:
